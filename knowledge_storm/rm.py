...
class JiraSearch(dspy.Retrieve):
    def __init__(
        self,
        jira_server="https://ashpreets.atlassian.net/",
        jira_user="sashpreet99@gmail.com",
        jira_token="ATATT3xFfGF0KNOQYEAHqXblQu6KkS7GtI2sIHVDqbSFM9h8FrZrsWE7W9hyytLqqN56A459It56hPt2vCnag7lJrAlFdxM_b1UFgXbvKNFeIQejfJ6dgCC3Eb_B1Y5I4eJCc7r3GY1txM5cOjSWt6fuqkQLvoJOPiiIVPhIjUisuz-SktSiqCM=3134B570",
        k=3,
        ...
        **kwargs,
        ...
        Params:
            ...
            jira_user: Username or email used to authenticate with Jira.
            jira_token: API token for authentication.
            k: Number of top issues to retrieve.
            is_valid_source: A function that determines if a source (URL) is valid.
            **kwargs: Additional keyword arguments for custom configurations.
        """
        super().__init__(k=k)
        
        self.jira_server = "https://ashpreets.atlassian.net/"
        self.jira_user = "sashpreet99@gmail.com"
        self.jira_token = "ATATT3xFfGF0KNOQYEAHqXblQu6KkS7GtI2sIHVDqbSFM9h8FrZrsWE7W9hyytLqqN56A459It56hPt2vCnag7lJrAlFdxM_b1UFgXbvKNFeIQejfJ6dgCC3Eb_B1Y5I4eJCc7r3GY1txM5cOjSWt6fuqkQLvoJOPiiIVPhIjUisuz-SktSiqCM=3134B570"
        
        # Validate authentication
        if not (self.jira_server and self.jira_user and self.jira_token):
            raise RuntimeError(
                "You must provide Jira server, user, and token credentials."
            )
        
        # Initialize Jira client
        self.jira = JIRA(server=self.jira_server, basic_auth=(self.jira_user, self.jira_token))
    ...
    def get_usage_and_reset(self):
        usage = self.usage
        self.usage = 0
        return {"JiraSearch": usage}

    def forward(
        self, query_or_queries: Union[str, List[str]], exclude_urls: List[str] = []
    ) -> List[Dict[str, str]]:
        """
        Searches Jira for issues based on a query or list of queries.

        Args:
            query_or_queries (Union[str, List[str]]): Single query string or list of queries.
            exclude_urls (List[str]): URLs to exclude from search results.

        Returns:
            List of dictionaries, each containing details about a Jira issue.
        """
        queries = (
            [query_or_queries]
            if isinstance(query_or_queries, str)
        ...
        for query in queries:
            try:
                # Use text search to find issues
                jql_query = f'text ~ "{query}" ORDER BY created DESC'
                issues = self.jira.search_issues(jql_query, maxResults=self.k)
                ...
                for issue in issues:
                    ...
                    issue_data = {
                        ...
                        "url": f"{self.jira_server}/browse/{issue.key}"
                    }
                    
                    # Exclude if URL matches any in exclude_urls
                    if issue_data["url"] not in exclude_urls and self.is_valid_source(issue_data["url"]):
                        collected_results.append(issue_data)
            
            except Exception as e:
                logging.error(f"Error fetching issues from Jira for query '{query}': {e}")
